...
export default function FileUploadZone() {
  ...
  return (
    <div className="space-y-4">
      <Card
        className={`relative border-[3px] border-dashed p-10 text-center transition-all duration-300 overflow-hidden glass-card ${
          isDragging
      ...
      >
        ...
        <div className="relative z-10">
          <Upload className={`h-20 w-20 mx-auto mb-6 transition-transform duration-300 ${
            isDragging ? 'text-primary scale-125 drop-shadow-lg' : 'text-primary/80 drop-shadow-md'
          }`} />
          <p className="text-2xl font-bold mb-3 text-foreground drop-shadow-sm">
            Drag and drop files here, or click to select
          </p>
          <p className="text-base text-foreground/80 mb-8 font-medium">
            Supports images, videos, documents, APKs, and more (max 100MB per file)
          </p>
          <input
            type="file"
            id="file-upload"
      ...
      {files.length > 0 && (
        <div className="space-y-3">
          <h3 className="font-bold text-lg text-white">Selected Files ({files.length})</h3>
          <div className="space-y-2">
            {files.map((file) => (
              <Card key={file.id} className="glass-card p-4 border-2 border-white/20 shadow-colorful hover:border-white/40 transition-all duration-300">
